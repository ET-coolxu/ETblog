...
- [x] 已发布点「存档」（确认后）：frontmatter `archived: true` 且无 `draft`；访客列表/搜索/RSS/sitemap 没有该文；直链 404；后台显示已存档
- [x] 已存档编辑页没有「发布」；点「发布」的伪造请求被拒绝并提示须先改为草稿
- [x] 已存档「改为草稿」后：`draft: true`、无 `archived`；访客仍 404；随后可发布，发布后前台可见
- [x] 已发布「改为草稿」或「保存草稿」后：访客 404，后台为草稿；可再发布
- [x] 草稿编辑页无「存档」；伪造存档请求被拒绝
- [x] 草稿 / 已发布 / 已存档均可删除；确认文案说明不可恢复、会删 PV、不删图片
- [x] 删除后 Markdown 文件不在；该 slug 的 `post_views` 行不在；`public/uploads/` 未因本次删除少文件
- [x] 删除当前最大序号的文章后，下一篇新建分配到的数字可以再次用到该序号；不存在「已删 slug 永久占用」
- [x] 后台列表三种状态文案正确；无文章仍有中文空状态
- [x] 存档文的 `featured` 不出现在首页精选
- [x] 管理员打开存档或草稿正文不记 PV；删除后统计总 PV 不再含该 slug
...
- 无法用浏览器验证的部分：伪造已存档「发布」、草稿「存档」的请求（可用 Server Action 校验的单测思路，或实现时写清已在 action 分支拒绝）。
  - 已在 `savePost` / `assertSaveTransition` 拒绝：已存档 + 发布 →「请先改为草稿再发布。」；草稿或新建 + 存档 →「草稿不能存档。」未知 `intent` 在 action 里返回「未知操作。」
